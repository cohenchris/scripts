#!/usr/bin/env bash
#
# Disaster recovery script for the 'files' ZFS pool (Nextcloud external
# storage).
#
# Usage:
#   files.sh fresh    # drive is gone - rebuild the pool from scratch
#   files.sh replace  # swap a still-working drive for a new one
#
# Both actions are destructive to the device(s) they operate on and will
# prompt for confirmation before wiping anything.
#
# This script can also be sourced instead (`source files.sh`) to pull in
...
# Ownership to restore on the pool's mountpoint after creation (Nextcloud
# external storage is served by the 'http' user/group).
OWNER="http"
GROUP="http"
...
# BORG RESTORE CONFIG  #
########################
# See ../../backup/files.sh and ../../backup/.env for how this repo is
# created/maintained. You will be prompted for the borg passphrase by borg
# itself when recreate_from_nothing() runs.
...
#
# Rebuilds the 'files' pool from a blank/replacement drive and restores its
# data from the local borg backup. Requires the 'backups' pool to already be
# restored (or point BORG_REPO at REMOTE_BORG_REPO above).
function recreate_from_nothing()
{
  ...
  if [[ "${BORG_REPO}" != *:* && ! -d "${BORG_REPO}" ]]; then
    echo "ERROR: borg repo ${BORG_REPO} not found - restore/recreate the 'backups' pool first, or point BORG_REPO at REMOTE_BORG_REPO"
    return 1
  fi
  ...
  echo "Restoring '${POOL_NAME}' data from borg backup (${BORG_REPO})..."
  echo "You will be prompted for the borg passphrase below (possibly more than once)."

  local archive
  archive="$(borg list --last 1 --short "${BORG_REPO}")"
  if [[ -z "${archive}" ]]; then
    echo "ERROR: no archives found in ${BORG_REPO}"
    return 1
  fi

  # Archive was created from the absolute path /files, so extract with cwd=/
  # to restore it back to /files
  echo "Extracting archive '${archive}'..."
  (cd / && run borg extract "${BORG_REPO}::${archive}")
  [[ $? -ne 0 ]] && echo "ERROR: borg extract failed" && return 1

  run chown -R "${OWNER}:${GROUP}" "${MOUNTPOINT}"

  echo "Done. Pool '${POOL_NAME}' is ready and data restored from archive '${archive}'."
}
